...
{
	I2C_Error_t res = I2C_Transmit(I2C_ID, ADDR, buff, len, I2C_DEFAULT_TIME_OUT);
	if(res != I2C_ERROR_NONE)
		Trace(1, "I2C%d write err: %d", I2C_ID, res);
}

static void i2cRead(void* buff, uint8_t len)
{
	I2C_Error_t res = I2C_Receive(I2C_ID, ADDR, buff, len, I2C_DEFAULT_TIME_OUT);
	if(res != I2C_ERROR_NONE)
		Trace(1, "I2C%d write err: %d", I2C_ID, res);
}
...
float bme280_readAltitude(float seaLevel)
{
  ...
  float atmospheric = (bme280_readPressure() / 256.0F) / 100.0F;
